...
test('Code 工具编排只生成审批提案，不直接执行', async () => {
  const proposal = await createCodeToolProposal({ mode: 'Code', tool: 'command', root: process.cwd(), input: { sessionId: 'code-session', argv: [process.execPath, '-e', 'console.log("ok")'] } });
  assert.equal(proposal.action.status, 'awaiting_approval');
  assert.deepEqual(proposal.command.argv.slice(0, 1), [process.execPath]);
});
