...
| MCP | `openclaw mcp` 支持 server/client registry、status/doctor/probe、Control UI `/mcp`；`serve` 为 stdio MCP server | 已有审批门禁注册骨架和显式 `shell:false` stdio JSON-RPC transport；不读取 SecretRef、不自动重启、不执行工具；SSE/streamable HTTP 与 OpenClaw 私有协议仍未接入 |
